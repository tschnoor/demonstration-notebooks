function [areas, M, qnew] = SensitiveTalker_Norm_SBranch_AltCalc(nareas,E,P,dx,q,cflag);
%SensitiveTalker.m
%B. Story
...
%
[i,jorder] = sort(P(4,:),'ascend');

%---Reorder E and P according the sorting operation above---
Esort = E(:,jorder);
Psort = P(:,jorder);

if(length(cflag) == 1)
    cflag = cflag*ones(1,size(Esort,2));
end

cflagsort = cflag(jorder);
...
for n=1:N
    ...
    for k=1:size(Esort,2)
        
        ar_thresh = 0.05; %added on 07.05.18; need to understand better why this works
        if(min(arN)>=ar_thresh & Esort(n,k) >= deltaE)
            [arN,y,d,g] = DeformationGenerator_Norm_SBranch(Esort(n,k),arN,dx,Psort(:,k),cflagsort(k),q.piri,LocPiri);
            Mtmp(k).Y(n,:) = y;
            Mtmp(k).C(n,:) = d;
        end
