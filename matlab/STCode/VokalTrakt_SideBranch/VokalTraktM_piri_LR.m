function [h,z,f,D,h_alt,Zrad] = VokalTraktM_piri_LR(ARvect,LTvect,cutoff,maxF, Apr,Lpr,Nr_piri, Apl,Lpl,Nl_piri)
...
% piriform right
[Zpr, fpr] = piri_imped(Apr(:),Lpr(:),maxF);
KprA = ones(NPoints,1);
KprB = zeros(NPoints,1);
KprC = -1 ./(Zpr);
KprD = ones(NPoints,1);

% piriform left
[Zpl, fpl] = piri_imped(Apl(:),Lpl(:),maxF);
KplA = ones(NPoints,1);
KplB = zeros(NPoints,1);
KplC = -1 ./(Zpl);
KplD = ones(NPoints,1);
...
% right piriform branch---------------------------------

M1 		= [A B; C D];
M2 		= [KprA KprB; KprC KprD];
